...
## 16. Super Agents and Confounders: Influence of surrounding agents on vehicle trajectory prediction
我们不应只重复这篇论文已有的工作，例如：再算一次Shaple，再做一次Agent删除，再比较Super和人工Causal标签。真正可以继续推进的是把评价从：删除Agent后，Loss变好还是变差扩展为改变Agent状态后，主角预测如何响应。
<img width="1173" height="369" alt="image" src="https://github.com/user-attachments/assets/8864f473-8bbd-4ec7-8abc-dfc46693369a" />

## 17. CounterScene: Counterfactual Causal Reasoning in Generative World Models for Safety-Critical Closed-Loop Evaluation
CounterScene已经做了：
-单Agent选择；
-时空连续干预；
-闭环响应；
...
所以我们不能只提出“连续改变Agent速度，看ego预测是否变化”，否则会与它明显重合。

但CounterScene的目标是：生成尽可能真实的危险场景。我们更可能做的是：评估一个轨迹预测模型对受控因果干预的响应是否正确。

它没有系统评价：
-不同预测模型面对同一个干预时，谁的响应方向更合理；
-响应是否随干预强度单调变化；
-非因果Agent扰动是否保持不变；
-模型内部Attention、Shapley和外部反事实响应是否一致；
-响应是否跨训练种子稳定。

因此它为我们提供了很重要的干预构造方法，但没有完成我们设想中的跨模型因果响应评估体系。

## 18. CADET: Physics-Grounded Causal Auditing and Training-Free Deconfounding of End-to-End Driving Planners
CADET最大的局限，是它把“谁是真因果Agent”的判断主要交给了手工物理先验：TTC、是否进入ego走廊、位置速度、类别和检测置信度。这套规则擅长识别短时、直接交互，但容易漏掉远期切入、间接因果链、多Agent联合影响，也会继承感知模块的位置和速度估计误差。因此，它更准确地是在发现“模型依赖与短时物理规则不一致的Agent”，还不能提供严格的真实因果GT。

它的实验范围也比较有限：真实部分只审计了SparseDrive、nuScenes-mini的81帧，而且主要是单Agent query置零、单帧、open-loop测试。CSI主要评价无关Agent扰动后是否稳定，CRI主要判断因果干预后的响应方向是否正确，但没有充分评价响应幅度、连续干预下的单调性、过度或不足反应、多Agent组合效应以及跨模型稳定性。query级删除也不等于物理一致的场景级反事实，因为其他Agent和环境不会随之自然变化。

对我们的启示是：不应重复做一个简单的“单Agent删除＋CSI/CRI”。我们可以把CADET视为基础版尺子，在它之上重点补齐三部分：建立比瞬时TTC更可靠的多步因果参照；把二元CRI扩展成方向、幅度、单调性和剂量—响应评价；加入双Agent、多Agent、Agent—环境以及间接因果链，并用同一协议横向测试QCNet、HPNet、MTR等预测模型。Training-free masking可以作为证明尺子具有实际修复价值的附加实验，但不应抢走第一篇“构建可靠评测体系”的主线。




## Summary_20260725
A. 强轨迹预测器：作为我们未来的被测模型
| 作品               | 核心任务                         | 核心思想                                                           | 与我们工作的启示性连接                                                                        |
| ---------------- | ---------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| **HiVT**         | 多模态轨迹预测                      | 采用层级式Vector Transformer：先建模Agent局部邻域，再进行场景级全局交互，在效率和交互表达之间取平衡。 | 代表“层级局部交互”范式。可以检验：模型缩小交互范围后，是否真的保留关键因果Agent，同时忽略远处无关Agent。                         |
| **QCNet**        | Query-centric边缘轨迹预测          | 所有Agent、地图元素均相对当前预测query进行编码，突出相对空间关系与坐标不变性。                   | 适合作为主力baseline。可以检查：高精度query-centric模型是否真正理解“谁影响目标Agent”，还是主要依赖道路结构和常见运动模式。        |
| **MTR**          | 多模态轨迹预测                      | 用静态意图query先定位可能终点，再通过动态搜索query进行局部运动细化。                        | 意图query为干预实验提供了额外观察对象：干预关键Agent后，是只改变轨迹细节，还是连意图模式和终点概率也会合理改变。                      |
| **MTR++**        | Joint multi-agent prediction | 在MTR基础上加入对称场景建模与引导式意图查询，同时联合预测多个Agent。                         | 比边缘预测更适合测试多Agent联合因果效应：两个Agent同时受干预时，模型能否产生相互一致的联合未来，而不是各自独立变化。                    |
| **Forecast-MAE** | 自监督预训练与轨迹预测                  | 通过遮蔽Agent历史、未来和地图信息进行重建预训练，学习更通用的场景表示。                         | 可检验“更强的自监督表示”是否自然带来更好的因果鲁棒性。它是很重要的反例候选：表示更鲁棒，不一定就更懂真实交互。                           |
| **HPNet**        | 利用历史预测增强当前预测                 | 把模型过去时刻生成的预测作为Prediction Memory，通过时间融合继续影响当前输出。                | 它为我们的benchmark增加了独特问题：虚假Agent或错误交互一旦进入历史预测，是否会被记忆模块不断传播和放大。可单独设计“历史预测污染与因果错误累积”测试。 |


B. 因果Agent标注、归因与模型诊断
| 作品                               | 核心任务                           | 核心思想                                                                                              | 与我们工作的启示性连接                                                                                                  |
| -------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| **CausalAgents**                 | 建立基于因果Agent标签的轨迹预测鲁棒性benchmark | 人工标注哪些周围Agent的存在会影响目标车辆行为，再分别删除因果与非因果Agent，观察预测性能变化。                                              | 是我们课题最直接的起点：证明不能只看ADE/FDE，还要看模型是否对正确Agent敏感。但它主要是二元标签＋删除干预，缺少影响方向、强度、单调性和多Agent效应。                           |
| **Super Agents and Confounders** | 分析社会信息到底帮助还是伤害轨迹预测             | 用Shapley attribution区分能改善指标的“Super Agents”和使指标恶化的“Confounders”，并比较不同随机种子的归因一致性；再用信息瓶颈压缩周围Agent信息。 | 直接启发我们加入**跨随机种子稳定性**和“内部归因—外部干预响应一致性”。它也说明更多社会信息不一定更好。但Super Agent是模型与指标依赖的事后定义，不等于真实因果Agent；信息瓶颈也未真正解决因果识别。 |

C. 在预测模型内部学习“因果表示”
| 作品                               | 核心任务               | 核心思想                                                                        | 与我们工作的启示性连接                                                                                                                |
| -------------------------------- | ------------------ | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **CaDeT：Causal Disentanglement** | 训练一个更鲁棒、泛化更好的轨迹预测器 | 在动态异构图表示上，将latent划分为因果模式与虚假模式；对虚假表示进行多次干预，并用不变性目标迫使预测主要依赖因果表示。              | 它说明“因果与虚假信息分离”可以作为训练目标。但它主要通过latent分解与鲁棒性结果间接证明因果性，未充分回答具体哪个Agent产生了什么方向和强度的作用。我们的benchmark可以作为外部审计工具，检验其因果声明是否真正反映在干预响应中。 |
| **Beyond Patterns**              | 通过因果推理增强轨迹预测       | 把空间地图和时序Agent信息置于因果图中，使用扩散式backdoor adjustment处理空间混杂，并通过反事实历史轨迹分支分离相关与因果作用。 | 它提示我们不仅要测试Agent，还要测试地图和历史信息的混杂作用。但其因果图与反事实构造由方法自身预设，评价仍主要依赖预测精度；我们的工作可独立检查其对地图扰动、Agent扰动和连续强度变化是否真的符合预期。                   |

D. 反事实场景生成与因果审计：最接近我们的工作
| 作品                                                       | 核心任务                                | 核心思想                                                                                     | 与我们工作的启示性连接                                                                                                                |
| -------------------------------------------------------- | ----------------------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **CounterScene**                                         | 为自动驾驶闭环测试生成真实而危险的反事实场景              | 先根据交通冲突选择关键对抗Agent，再通过空间冲突与到达时间压缩逐渐移除安全裕度；其他Agent由交互式world model继续演化。                    | 可以作为我们设计“场景级连续干预”的参考，尤其是速度、到达时间和冲突区进入程度的连续变化。但其目标是生成危险场景，而不是比较不同预测器的因果响应；Agent选择仍偏启发式，并以单Agent干预为主。                        |
| **CADET：Causal Auditing and Deconfounding at Test-time** | 对已经训练好的端到端规划器进行training-free因果审计和修复 | 用query消融测模型影响，用TTC、行驶走廊、类别和置信度构建物理先验，再与跨环境稳定性融合为PCR；提出CSI、CRI、CCS，并通过TCM在推理时mask可疑query。 | 它是与我们最接近的基础框架：CSI对应“无关因素不应影响”，CRI对应“因果因素应产生正确方向响应”。我们的空间主要在于把它从短时、单Agent、二元方向判断，扩展到连续剂量—响应、幅度校准、多Agent联合/间接因果和跨多个轨迹预测模型比较。 |
